...
def get_max_coordinate_range(point_cloud):
    ...
    # max_coords = max_coords.cpu()
    # # Convert the results to numpy arrays for easier manipulation
    # min_coords = min_coords.detach().numpy()
    # max_coords = max_coords.detach().numpy()
...
def BatchPC2Voxel(pc, save_path):
    ...
    max_num_voxels = int(np.prod(range_xyz)*10000)
    # OverflowError: cannot convert float infinity to integer
    # max_num_voxels = 1000000
    gen = PointToVoxel(
        vsize_xyz=[0.05, 0.05, 0.05],
        coors_range_xyz=coors_range_xyz,
        num_point_features=101,  # ? Related to feature channels
